(function () {
    ...
    anim = function (moveLeft, moveTop) {
        if (parseInt(baddie.style.left) + moveLeft > (container.clientWidth - baddie.clientWidth)
            || parseInt(baddie.style.left) + moveLeft < 0) {
            console.log('Out of bounds!');
        } else {
            baddie.style.left = (baddie.offsetLeft + moveLeft) + 'px';
        }
        if (parseInt(baddie.style.top) + moveTop > container.clientHeight - baddie.clientHeight
            || parseInt(baddie.style.top) + moveTop < 0) {
            console.log('Out of bounds!');
        } else {
    ...
    function checkKeys(e) {
        var key,
            myTimer; // eslint-disable-line no-unused-vars
