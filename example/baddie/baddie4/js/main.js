...
(function () {
    ...
    /**
     * Baddie - myBaddie
     * Baddie has: size, position in the grid (x,y), direction (left or right),
     * and an element in the DOM
    ...
    var myBaddie = {
        ...
        turn: function (x) {
            ...
            // ------------------------------
            // ASSIGNMENT
            // Use code from turnRight() and turnLeft() in baddie3 and rewrite
            // it to be based on if x is -1 or 1
         ...
         */
        move: function (pos) {
            // We check if x is different, because if it is we have a change in
            // horizontal movement - a turn
            if (this.x != pos.x) {
                // We want to build in turn() into the movement so we don't have to call it
                // ourselves in the appropriate switch-case
    ...
    /**
     * GameArea - myGameArea
     * GameArea has: size of each tile, size of the grid, tileMap (matrix of map content),
     * DOM element of area
    ...
    var myGameArea = {
        ...
        init: function () {
            ...
            console.log("Initiating game area size with %ix%i grid and tilesize of %ix%i px",
                this.gridSize, this.gridSize, this.tileSize, this.tileSize);
            this.element.style.width = this.element.style.height = this.gridSize*this.tileSize + "px"; // eslint-disable-line max-len
        ...
        draw: function () {
            ...
            var x, y, tile; // eslint-disable-line no-unused-vars
            ...
            for (y = 0; y < this.gridSize; y++) {
                for (x = 0; x < this.gridSize; x++) {
                    ...
                    // ------------------------------
                    // ASSIGNMENT
                    // Take part of the code from drawGamePlan() in baddie3 and
                    // rewrite it so that it draws the tiles into this.element
    ...
    */
     ...
     * @return {bool OR object} - if baddie was movable and position
     *         object (with x and y) is returned,
     *         otherwise false is returned
     * Changed from baddie3:
         * newLeft and newRight into object named newPos
         * Gets tile from tileMap in myGameArea-object instead of gameArea,
         *      uses newPos object for x and y
         * case 12:
    ...
    /*    var isBaddieMovable = function(moveLeft, moveTop){
        ...
        // Get the tile baddie wants to move to out of the array -
        //      this time it's a 2D array (arrays in array)
        ...
        switch(tile) {
            ...
            case 13: // door
                ...
                break;
            case 12:
                // When there is a box, we have to get the next position again
                //      to check if the box next to baddie can be moved
                // We use the same direction variables to create a new object for this position
                var nextPos = {
                    x: newPos.x + moveLeft,
                    y: newPos.y + moveTop
                };
                // We use the myGameArea object to check if we can move the
                //      box, allow baddie to move if it worked
    ...
    /* MAKING PUSHING A BOX POSSIBLE
        Notice that we have prepared by using a method in
        the isBaddieMovable-function above for this
    */
     ...
     * @param  {object} from    - the position to move to in the grid as an
     *                                  object containing x and y
     * @param  {object} to        - the position to move to in the grid as
     *                                  an object containing x and y
     * @return {boolean}         - returns true or false based on if tiles
     *                                  was moved or not
     */
    myGameArea.moveTile = function (from, to) {
        ...
        if (nextTile == 10) {
            ...
            console.log("Can't move tile, next tile (%ix%i) was not empty: %i",
                to.x, to.y, nextTile);
            console.groupEnd();
    ...
    // 2: Adding a method to refresh a tile that was changed so that
    //      it is redrawn (it's style changed)
    /**
    ...
    /* ------------------------------------
    /* ADDING ATTACK
        Notice that there already is a call to the baddieAttack()-
            function in the keylistener that has been
